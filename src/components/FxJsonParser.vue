<template>
    <div>
        <div class="twitter-table-container">
          ...
          >
            <!-- 用户信息列 -->
            <el-table-column label="用户信息" width="400">
            ...
            <!-- 推文内容列 -->
            <el-table-column label="推文内容" min-width="300">
              <template #default="{ row }">
              ...
              <template #default="{ row }">
                <div class="time-info">
                  <div v-if="row.user?.joined" class="join-time">
                    ...
                    加入时间：{{ formatDate(row.user.joined) }}
                  </div>
                  <div v-if="row.tweet?.created_at" class="tweet-time">
                    <el-icon><Clock /></el-icon>
                    发布时间：{{ formatDate(row.tweet.created_at) }}
                  </div>
                  <div v-if="row.tweet?.views" class="tweet-views">
                    <el-icon><View /></el-icon>
                    浏览量：{{ formatNumber(row.tweet.views) }}
                  </div>
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="120" fixed="right">
              <template #default="{ row }">
                <el-button
                  ...
                  type="primary"
                  size="small"
                  @click="openLink(row.user?.url || row.tweet?.url)"
                >
                  查看原文
                </el-button>
              </template>
